...
import dev.bpmcrafters.processengineapi.adapter.operaton.remote.task.delivery.*
  ...
  override fun refresh() {
    synchronized(refreshLock) {
      ...
      if (subscriptions.isNotEmpty()) {
        ...
        logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-036: pulling user tasks for subscriptions: $subscriptions" }
                ...
                executorService.submit {  // in another thread
                  try {
                    ...
                    if (taskInformation != null) {
                      ...
                      logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-037: delivering user task ${task.id}." }
                      activeSubscription.action.accept(taskInformation, variables)
                    } else {
                      logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-040: skipping task ${task.id} since it is unchanged." }
                    }
                  } catch (e: Exception) {
                    logger.error(e) { "PROCESS-ENGINE-OPERATON-REMOTE-038: error delivering task ${task.id}: ${e.message}" }
                    ...
                    synchronized(deliveredTaskIds) {
                      if (deliveredTaskIds.contains(task.id!!)) { // if the task was already there, remove it from unprocessed list
                        ...
                        if (!successful) {
                          logger.error { "PROCESS-ENGINE-OPERATON-REMOTE-038: error processing task ${task.id}, could not mark updated task as processed." }
                        }
                      }
                    }
                  }
        ...
        deliveredTaskIds.parallelStream().map { taskId ->
          executorService.submit {
            ...
            // deactivate active subscription and handle termination
            logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-042: deactivating $taskId, task is gone." }
            synchronized(deliveredTasks) {
        ...
        logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-039: pull user tasks disabled because of no active subscriptions" }
      }
    }
  }
  ...
  private fun cleanupFailedDelivery(
    ...
    if (activeSubscription != null) {
      try {
        ...
        logger.error(terminationError) { "PROCESS-ENGINE-OPERATON-REMOTE-045: error cleaning up failed delivery for task $taskId: ${terminationError.message}" }
      }
    }
  }
      ...
      .all {
        when (it.key) {
          ...
          CommonRestrictions.PROCESS_DEFINITION_VERSION_TAG -> it.value == processDefinitionMetaDataResolver.getProcessDefinitionVersionTag(task.processDefinitionId)
            ...
            logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-044: Unknown restriction key: ${it.key}" }
            false
          }
        }
      }
