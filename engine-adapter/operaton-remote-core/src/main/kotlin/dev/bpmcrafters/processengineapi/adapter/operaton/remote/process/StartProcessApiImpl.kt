...
import dev.bpmcrafters.processengineapi.adapter.operaton.remote.correlation.applyRestrictions
    ...
    return when (cmd) {
      is StartProcessByDefinitionCmd ->
        CompletableFuture.supplyAsync {
          logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-004: starting a new process instance by definition ${cmd.definitionKey}." }
      ...
      is StartProcessByMessageCmd ->
        CompletableFuture.supplyAsync {
          logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-005: starting a new process instance by message ${cmd.messageName}." }
          ensureSupported(cmd.restrictions)
          val payload = cmd.payloadSupplier.get()
          ...
          when (messageCorrelation.body?.size) {
            ...
            1 -> messageCorrelation.body?.get(0)?.toProcessInformation()
              ...
              logger.warn { "PROCESS-ENGINE-OPERATON-REMOTE-005: multiple results received, returning the first one." }
      ...
      is StartProcessByDefinitionAtElementCmd ->
        CompletableFuture.supplyAsync {
          logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-006: starting a new process instance by definition ${cmd.definitionKey} at element ${cmd.elementId}" }
          val payload = cmd.payloadSupplier.get()
          val tenantId = cmd.restrictions[CommonRestrictions.TENANT_ID]
          val processDefinitionId = getProcessDefinitionId(cmd.definitionKey, tenantId)
      ...
      is StartProcessByMessageAtElementCmd ->
        CompletableFuture.supplyAsync {
          logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-007: starting a new process instance by message ${cmd.messageName} at element ${cmd.elementId}" }
