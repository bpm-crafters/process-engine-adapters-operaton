...
class SubscribingServiceTaskDelivery(
  ...
  fun subscribe() {
    ...
    if (subscriptions.isNotEmpty()) {
      logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-030: subscribing to external tasks for: $subscriptions" }
              ...
              .handler { externalTask, externalTaskService ->
                if (subscription.matches(externalTask)) {
                  try {
                    ...
                    logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-031: delivering service task ${externalTask.id}." }
                    subscription.action.accept(externalTask.toTaskInformation().withReason(TaskInformation.CREATE), variables)
                    logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-032: successfully delivered service task ${externalTask.id}." }
                  } catch (e: Exception) {
                    val jobRetries: Int = externalTask.retries?.minus(1) ?: retries
                    logger.error { "PROCESS-ENGINE-OPERATON-REMOTE-033: failing delivering task ${externalTask.id}: ${e.message}" }
                    ...
                    logger.error { "PROCESS-ENGINE-OPERATON-REMOTE-034: successfully failed delivering task ${externalTask.id}: ${e.message}" }
                  }
                } else {
                  // put it back
                  externalTaskService.handleFailure(
                    externalTask.id,
                    "PROCESS-ENGINE-OPERATON-REMOTE-031: No matching handler",
                    "",
      ...
      logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-035: external tasks subscribing is disabled because of no active subscriptions" }
    }
  }
      ...
      .minus( // ignore some restrictions which are not relevant for external tasks
        ...
        when (it.key) {
          ...
          CommonRestrictions.PROCESS_DEFINITION_VERSION_TAG -> it.value == externalTask.processDefinitionVersionTag
            ...
            logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-045: Unknown restriction key: ${it.key}" }
            false
          }
        }
      }
