...
import dev.bpmcrafters.processengineapi.adapter.operaton.remote.task.delivery.RefreshableDelivery
import dev.bpmcrafters.processengineapi.adapter.operaton.remote.task.delivery.ServiceTaskDelivery
import dev.bpmcrafters.processengineapi.adapter.operaton.remote.task.delivery.pull.PullServiceTaskDeliveryMetrics.DropReason.EXPIRED_WHILE_IN_QUEUE
import dev.bpmcrafters.processengineapi.adapter.operaton.remote.task.delivery.pull.PullServiceTaskDeliveryMetrics.DropReason.NO_MATCHING_SUBSCRIPTIONS
import dev.bpmcrafters.processengineapi.adapter.operaton.remote.task.delivery.pull.PullServiceTaskDeliveryMetrics.FetchAndLockSkipReason.NO_SUBSCRIPTIONS
import dev.bpmcrafters.processengineapi.adapter.operaton.remote.task.delivery.pull.PullServiceTaskDeliveryMetrics.FetchAndLockSkipReason.QUEUE_FULL
import dev.bpmcrafters.processengineapi.adapter.operaton.remote.task.delivery.toTaskInformation
...
class PullServiceTaskDelivery(
  ...
  internal fun deliverNewTasks() {
    ...
    if (subscriptions.isEmpty()) {
      logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-035: Pull external tasks disabled because of no active subscriptions" }
    ...
    if (tasksToFetch == 0) {
      logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-041: Task executor queue is full, skipping task fetch" }
      metrics.incrementFetchAndLockTasksSkippedCounter(QUEUE_FULL)
      return
    }

    logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-030: pulling $tasksToFetch service tasks for subscriptions: $subscriptions" }
    ...
    logger.trace { "PROCESS-ENGINE-OPERATON-REMOTE-042: pulled ${lockedTasks.size} service tasks" }
    lockedTasks
  ...
  internal fun createTaskActionHandlerCallable(lockedTask: LockedExternalTaskDto, activeSubscription: TaskSubscriptionHandle): Callable<Unit> = Callable {
    ...
    if (start.isBefore(lockedTask.lockExpirationTime)) {
      try {
        ...
        logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-031: delivering service task ${lockedTask.id}." }
        ...
        logger.error { "PROCESS-ENGINE-OPERATON-REMOTE-033: failing delivering task ${lockedTask.id}: ${e.message}" }
        ...
        logger.error { "PROCESS-ENGINE-OPERATON-REMOTE-034: successfully failed delivering task ${lockedTask.id}: ${e.message}" }
      } finally {
  ...
  private fun cleanupFailedDelivery(taskId: String) {
    ...
    if (taskSubscriptionHandle != null) {
      try {
        ...
        logger.error { "PROCESS-ENGINE-OPERATON-REMOTE-046: failed cleaning up delivery state for task $taskId: ${terminationError.message}" }
      }
    }
  }
      ...
      .minus( // ignore some restrictions which are not relevant for external tasks
        ...
        when (it.key) {
          ...
          CommonRestrictions.PROCESS_DEFINITION_VERSION_TAG -> it.value == task.processDefinitionVersionTag
            ...
            logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-043: Unknown restriction key: ${it.key}" }
            false
          }
        }
      }
