...
import dev.bpmcrafters.processengineapi.adapter.operaton.embedded.task.delivery.*
  ...
  override fun refresh() {
    synchronized(refreshLock) {
      ...
      if (subscriptions.isNotEmpty()) {
        ...
        logger.trace { "PROCESS-ENGINE-OPERATON-EMBEDDED-036: pulling user tasks for subscriptions: $subscriptions" }
        taskService
          .createTaskQuery()
          .initializeFormKeys()
                ...
                executorService.submit {  // in another thread
                  try {
                    ...
                    if (taskInformation != null) {
                      ...
                      logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-037: delivering user task ${task.id}." }
                      activeSubscription.action.accept(taskInformation, variables)
                    } else {
                      logger.trace { "PROCESS-ENGINE-OPERATON-EMBEDDED-040: skipping task ${task.id} since it is unchanged." }
                    }
                  } catch (e: Exception) {
                    logger.error(e) { "PROCESS-ENGINE-OPERATON-EMBEDDED-038: error delivering task ${task.id}: ${e.message}" }
                    ...
                    synchronized(deliveredTaskIds) {
                      if (deliveredTaskIds.contains(task.id)) { // if the task was already there, remove it from unprocessed list
                        ...
                        if (!successful) {
                          logger.error { "PROCESS-ENGINE-OPERATON-EMBEDDED-038: error processing task ${task.id}, could not mark updated task as processed." }
                        }
                      }
                    }
                  }
        ...
        deliveredTaskIds
          ...
          .map { taskId ->
            executorService.submit { // also async
              ...
              logger.trace { "PROCESS-ENGINE-OPERATON-EMBEDDED-042: deactivating $taskId, task is gone." }
        ...
        logger.trace { "PROCESS-ENGINE-OPERATON-EMBEDDED-039: pull user tasks disabled because of no active subscriptions" }
      }
    }
  }
  ...
  private fun cleanupFailedDelivery(taskId: String) {
    ...
    if (activeSubscription != null) {
      try {
        ...
        logger.error(terminationError) { "PROCESS-ENGINE-OPERATON-EMBEDDED-044: error cleaning up failed delivery for task $taskId: ${terminationError.message}" }
      }
    }
  }
      ...
      .all {
        when (it.key) {
          ...
          CommonRestrictions.PROCESS_DEFINITION_VERSION_TAG -> it.value == processDefinitionMetaDataResolver.getProcessDefinitionVersionTag(task.processDefinitionId)
            ...
            logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-043: Unknown restriction key: ${it.key}" }
            false
          }
        }
      }
