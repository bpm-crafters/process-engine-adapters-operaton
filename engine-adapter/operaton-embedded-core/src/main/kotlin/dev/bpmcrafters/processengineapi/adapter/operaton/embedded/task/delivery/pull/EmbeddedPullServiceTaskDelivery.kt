...
import dev.bpmcrafters.processengineapi.adapter.operaton.embedded.task.delivery.ExternalServiceTaskDelivery
import dev.bpmcrafters.processengineapi.adapter.operaton.embedded.task.delivery.pull.EmbeddedPullServiceTaskDeliveryMetrics.DropReason.EXPIRED_WHILE_IN_QUEUE
import dev.bpmcrafters.processengineapi.adapter.operaton.embedded.task.delivery.pull.EmbeddedPullServiceTaskDeliveryMetrics.DropReason.NO_MATCHING_SUBSCRIPTIONS
import dev.bpmcrafters.processengineapi.adapter.operaton.embedded.task.delivery.pull.EmbeddedPullServiceTaskDeliveryMetrics.FetchAndLockSkipReason.NO_SUBSCRIPTIONS
import dev.bpmcrafters.processengineapi.adapter.operaton.embedded.task.delivery.pull.EmbeddedPullServiceTaskDeliveryMetrics.FetchAndLockSkipReason.QUEUE_FULL
import dev.bpmcrafters.processengineapi.adapter.operaton.embedded.task.delivery.toTaskInformation
...
class EmbeddedPullServiceTaskDelivery(
  ...
  internal fun deliverNewTasks() {
    ...
    if (subscriptions.isEmpty()) {
      logger.trace { "PROCESS-ENGINE-OPERATON-EMBEDDED-035: Pull external tasks disabled because of no active subscriptions" }
    ...
    if (tasksToFetch == 0) {
      logger.trace { "PROCESS-ENGINE-OPERATON-EMBEDDED-044: Task executor queue is full, skipping task fetch" }
      metrics.incrementFetchAndLockTasksSkippedCounter(QUEUE_FULL)
      return
    }

    logger.trace { "PROCESS-ENGINE-OPERATON-EMBEDDED-030: pulling $tasksToFetch service tasks for subscriptions: $subscriptions" }
    val lockedTasks = externalTaskService
      .fetchAndLock(tasksToFetch, workerId)
      .forSubscriptions(subscriptions)
      .execute()

    logger.trace { "PROCESS-ENGINE-OPERATON-EMBEDDED-045: pulled ${lockedTasks.size} service tasks" }
    lockedTasks
      ...
      if (start.isBefore(lockExpirationInstant)) {
        try {
          if (subscriptionRepository.getActiveSubscriptionForTask(lockedTask.id) == activeSubscription) {
            ...
            logger.trace { "PROCESS-ENGINE-OPERATON-EMBEDDED-041: skipping task ${lockedTask.id} since it is unchanged." }
          } else {
            ...
            logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-031: delivering service task ${lockedTask.id}." }
            activeSubscription.action.accept(taskInformation, variables)
            logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-032: successfully delivered service task ${lockedTask.id}." }
          }
          logger.debug { "PROCESS-ENGINE-OPERATON-REMOTE-032: successfully delivered service task ${lockedTask.id}." }
          metrics.incrementCompletedTasksCounter(lockedTask.topicName!!)
        } catch (e: Exception) {
          val jobRetries: Int = lockedTask.retries?.minus(1) ?: retries
          logger.error { "PROCESS-ENGINE-OPERATON-EMBEDDED-033: failing delivering task ${lockedTask.id}: ${e.message}" }
          ...
          logger.error { "PROCESS-ENGINE-OPERATON-EMBEDDED-034: successfully failed delivering task ${lockedTask.id}: ${e.message}" }
  ...
  private fun cleanupFailedDelivery(taskId: String) {
    ...
    if (taskSubscriptionHandle != null) {
      try {
        ...
        logger.error { "PROCESS-ENGINE-OPERATON-EMBEDDED-046: failed cleaning up delivery state for task $taskId: ${terminationError.message}" }
      }
    }
  }
      ...
      .all {
        when (it.key) {
          ...
          CommonRestrictions.PROCESS_DEFINITION_VERSION_TAG -> it.value == task.processDefinitionVersionTag
            ...
            logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-041: Unknown restriction key: ${it.key}" }
            false
          }
        }
      }
