...
import dev.bpmcrafters.processengineapi.adapter.operaton.embedded.correlation.applyTenantRestrictions
    ...
    return when (cmd) {
      is StartProcessByDefinitionCmd ->
        commandExecutor.execute {
          logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-004: starting a new process instance by definition ${cmd.definitionKey}." }
      ...
      is StartProcessByMessageCmd ->
        commandExecutor.execute {
          logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-005: starting a new process instance by message ${cmd.messageName}." }
          val payload = cmd.payloadSupplier.get()
      ...
      is StartProcessByDefinitionAtElementCmd ->
        commandExecutor.execute {
          logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-006: starting a new process instance by definition ${cmd.definitionKey} at element ${cmd.elementId}" }
      ...
      is StartProcessByMessageAtElementCmd ->
        commandExecutor.execute {
          logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-007: starting a new process instance by message ${cmd.messageName} at element ${cmd.elementId}" }
