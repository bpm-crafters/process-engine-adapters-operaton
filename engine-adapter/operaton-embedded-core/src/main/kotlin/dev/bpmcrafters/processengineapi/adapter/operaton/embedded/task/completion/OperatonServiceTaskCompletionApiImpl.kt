...
class OperatonServiceTaskCompletionApiImpl(
  ...
  override fun completeTask(cmd: CompleteTaskCmd): CompletableFuture<Empty> {
    ...
    logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-006: completing service task ${cmd.taskId}." }
    return commandExecutor.execute {
      ...
      subscriptionRepository.deactivateSubscriptionForTask(cmd.taskId)?.apply {
        ...
        logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-007: successfully completed service task ${cmd.taskId}." }
      }
      Empty
    }
  }
  ...
  override fun completeTaskByError(cmd: CompleteTaskByErrorCmd): CompletableFuture<Empty> {
    logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-008: throwing error ${cmd.errorCode} in service task ${cmd.taskId}." }
    return commandExecutor.execute {
      ...
      subscriptionRepository.deactivateSubscriptionForTask(cmd.taskId)?.apply {
        termination.accept(TaskInformation(cmd.taskId, emptyMap()).withReason(TaskInformation.COMPLETE))
        logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-009: successfully thrown error in service task ${cmd.taskId}." }
      }
      Empty
    }
  }
  ...
  override fun failTask(cmd: FailTaskCmd): CompletableFuture<Empty> {
    logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-010: failing service task ${cmd.taskId}." }
    return commandExecutor.execute {
      ...
      subscriptionRepository.deactivateSubscriptionForTask(cmd.taskId)?.apply {
        termination.accept(TaskInformation(cmd.taskId, emptyMap()).withReason(TaskInformation.COMPLETE))
        logger.debug { "PROCESS-ENGINE-OPERATON-EMBEDDED-011: successfully failed service task ${cmd.taskId} handling." }
      }
      Empty
    }
  }
